...
const grandParentRoot = path.resolve(parentRoot, '..')
const imagesRoot = path.resolve(grandParentRoot, 'images')
const siteConfigPath = path.resolve(projectRoot, './.figma/make/site.json')
const siteConfiguration = loadSiteConfiguration(siteConfigPath)

function loadSiteConfiguration(filePath: string): FigmaSiteConfiguration {
  try {
    ...
    return raw ? (JSON.parse(raw) as FigmaSiteConfiguration) : {}
  } catch (error) {
    console.warn(`Could not load Figma site configuration from ${filePath}:`, error)
    return {}
  }
}
...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      fs: {
        allow: [projectRoot, parentRoot, grandParentRoot, imagesRoot],
      },
      watch: { ignored: ['**/.figma/**'] },
    },
    preview: {
